...
var LDAvis = function(to_select, data_or_file_name) {
    ...
    //console.log("data= "+data_or_file_name['mdsDat']);
    var current_clicked = {
        what: "nothing",
        element: undefined
    },
    ...
    var valName= topicID+"-changeName";
    var valFusion = topicID+"-fusion";
    ...
    function download(filename) {
        ...
        //element.setAttribute('href', 'data:text/plain;charset=utf-8,' + encodeURIComponent(JSON.stringify(ldavis_el434344057280804269385084_data)));
        //element.setAttribute('href', 'data:text/plain;charset=utf-8,' + encodeURIComponent("teststststs"));

        element.setAttribute('href',"data:text/plain;charset=utf-8,"+encodeURIComponent("coucou c'est moi !!!"));
    ...
    function visualize(data) {
        ...
        d3.select("#" + valName)
            .on("click", function() {
                ...
                // remove termselection if it exists (from a saved URL)
                //var termElem = document.getElementById(termID + vis_state.term);
                 
                var value_old = document.getElementById(topicID).value;
                //var t=document.getElementById(topicID + value_old);

                //topic_off(document.getElementById(topicID + value_old));

                console.log(to_select);
                var text = d3.select(to_select + " .bubble-tool");
                var newName = document.getElementById("nameTopic").value;
                //var x = document.getElementsByClassName("bubble-tool");
                //console.log(x[0]);
                tabName[document.getElementById(topicID).value]=newName;
                //console.log("id topic="+topicID+", name="+text.text());
                ...
                .text(" Most Relevant Terms for Topic " + newName );
        ...
        d3.select("#" + valFusion)
            .on("click", function() {
                ...
                var tmp2=document.getElementById("fusion2").value;
        ...
        function init_forms(topicID, lambdaID, visID) {
            ...
            var topicName = document.createElement("name");
            topicName.setAttribute("for", topicID);
            topicName.setAttribute("style", "font-family: sans-serif; font-size: 14px");
            topicName.innerHTML = "Change Name Topic: <span id='" + topicID + "-value'></span>";
            ...
            topicNameInput.setAttribute("style", "width: 170px");
            topicNameInput.type = "text";
            topicNameInput.step = "1";
            topicNameInput.id = "nameTopic";
            topicDiv.appendChild(topicNameInput);

            var validName = document.createElement("button");
            validName.setAttribute("id", valName);
            validName.setAttribute("style", "margin-left: 5px");
            validName.innerHTML = "Valid";
            topicDiv.appendChild(validName);
            ...
            var fusionTopic = document.createElement("name");
            fusionTopic.setAttribute("for", topicID);
            fusionTopic.setAttribute("style", "font-family: sans-serif; font-size: 14px");
            fusionTopic.innerHTML = "Fusion 2 Topics: <span id='" + topicID + "-value'></span>";
            topicDiv.appendChild(fusionTopic);



            var topicF1 = document.createElement("input");
            topicF1.setAttribute("style", "width: 150px");
            topicF1.type = "text";
            topicF1.step = "1";
            topicF1.value="topic 1";
            topicF1.id = "fusion1";
            topicDiv.appendChild(topicF1);


            var topicF2 = document.createElement("input");
            topicF2.setAttribute("style", "width: 150px");
            topicF2.type = "text";
            topicF2.step = "1";
            topicF2.value="topic 2"
            topicF2.id = "fusion2";
            topicDiv.appendChild(topicF2);

            var validFusion = document.createElement("button");
            validFusion.setAttribute("id", valFusion);
            validFusion.setAttribute("style", "margin-left: 5px");
            validFusion.innerHTML = "Valid";
            topicDiv.appendChild(validFusion);
        ...
        function topic_on(circle) {
...

            {
                ...
                d3.select("#" + barFreqsID)
                    ...
                    .style("text-anchor", "middle")
                    .style("font-size", "16px")
                    .text("Top-" + R + " Most Relevant Terms for Topic " + tabName[topics] + " (" + Freq + "% of tokens)");
...
var str="";
    for(var key in tabName)
    { 
          str=str+" "+key+"-"+tabName[key];
    }
    str=str+"#";
    for(var key in tabFusion)
    { 
          str=str+" "+key+"-"+tabFusion[key];
    }
    return str;
}
